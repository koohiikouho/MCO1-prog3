...
public class ManageHotel {
     ...
     * @param hotels imports hotel arrayList
     * @return hotelNum which is the index of the hotel the User picked, if there are none it returns a negative value 
     */
		...
		if (hotelNum >= 0) 
		{
			if(hotels.get(hotelNum).rooms.size() < 50)
			{
				...
				do
				{
					...
					{
						if(numRoom == hotels.get(hotelNum).rooms.get(i).getRoomNumber() && numFloor == hotels.get(hotelNum).rooms.get(i).getRoomFloor())
						{
	...
	/**
     * removeReservation allows the user to remove reservations from the reservation arrayList
     * 
     * @param scan   imports scanner
     * @param hotels imports hotel arrayList
     * @param hotelNum imports the index of the user's chosen hotel
