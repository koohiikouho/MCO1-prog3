			...
			if (hotels.size() != 0) {
				...
				if (reserveInd != -1) {
					...
					do {
						...
						if (year > 2050)
							System.out.println(
									"I think thats a bit too far into the future, try again this time smaller");
					...
					hotels.get(hotelNum).getReservations().add(
							new Reservation(name, checkInDate, checkOutDate, hotels.get(hotelNum).rooms.get(roomNum)));
